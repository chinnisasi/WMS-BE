...
export class CatalogController {
  ...
  @ApiHeaders(IDEMPOTENCY_HEADER)
  @ApiOkResponse({ type: ProductResponse, description: 'The created product (a matching Idempotency-Key replays it)' })
  @ApiResponse({ status: 400, ...problemJsonResponse('Missing or malformed Idempotency-Key, or an invalid name/axes (validation-failed / empty-product-edit)') })
  @ApiResponse({ status: 401, ...problemJsonResponse('Missing or invalid session token') })
