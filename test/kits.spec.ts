...
describe('kits: kit_compositions, the never-independent-stock guards, order explosion (e2e, story 11.4)', () => {
  ...
  describe('a kit never holds stock', () => {
    ...
    // ── fix A2: the kit-create write-skew races ─────────────────────────────
    // Both races drive two REAL concurrent requests at one SKU row and assert
    // the EITHER/OR safe outcome — never both committed. Before fix A2 only
    // the GRN took the SKU row lock, so a kit create that read stock = 0 and
    // a concurrent +stock writer that read not-a-kit could both commit,
